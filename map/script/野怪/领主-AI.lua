...
ac.game:event '受到伤害开始' (function(trg, damage)
	...
	if source:is_type('领主') and source:get_point() * source:getBornPoint() < creep_range then
		source:remove_buff '领主脱战回血'
		return
	end
	if not target:is_type('领主') then
		return
	end
