...
class.today_panel = extends(class.panel){
    create = function()
        ...
        local panel = class.panel.create('image\\提示框\\bj2.tga',470+475+30,200,475,620)
        panel.__index = class.today_panel 
        
        --头部
        local top_panel = panel:add_panel('',0,20,475,100)
        --标题 (title,0,0,width,height,font_size,4)
        local title = top_panel:add_text('无尽排行榜(今日)',0,0,475,100,16,4)
        ...
        --段位
        local gold = data_panel:add_text('无尽层数',160,0,80,20,10,1)
