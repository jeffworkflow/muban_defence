...
cnt_succ = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总通关次数') - (p.cus_server['S6通关次数'] or 0)
    cnt = math.min(cnt,500,p:Map_GetMapLevel()*25) --500，25
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
cnt_ljwj = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总无尽累计') - (p.cus_server['S6无尽累计'] or 0)
    cnt = math.min(cnt,1500,p:Map_GetMapLevel()*100) --1500,100
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
cnt_wbjf = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总挖宝积分') - (p.cus_server['S6挖宝积分'] or 0)
    cnt = math.min(cnt,5000,p:Map_GetMapLevel()*500) --5000,500
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
cnt_wljf = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总比武积分') - (p.cus_server['S6比武积分'] or 0)
    cnt = math.min(cnt,2500,p:Map_GetMapLevel()*50) 
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
cnt_succ = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总通关次数') - (p.cus_server['S7通关次数'] or 0)
    cnt = math.min(cnt,500,p:Map_GetMapLevel()*25) --500，25
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
cnt_ljwj = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总无尽累计') - (p.cus_server['S7无尽累计'] or 0)
    cnt = math.min(cnt,1500,p:Map_GetMapLevel()*100) --1500,100
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
cnt_wbjf = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总挖宝积分') - (p.cus_server['S7挖宝积分'] or 0)
    cnt = math.min(cnt,5000,p:Map_GetMapLevel()*500) --5000,500
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
cnt_wljf = function(self)
    local hero = self.owner
    local p = hero:get_owner()
    local cnt = get_season(p,'总比武积分') - (p.cus_server['S7比武积分'] or 0)
    cnt = math.min(cnt,2500,p:Map_GetMapLevel()*50) 
    cnt = cnt > 0 and cnt or 0
    return cnt
end,
...
for i,name in ipairs({'S0赛季','S1赛季','S2赛季','S3赛季','S4赛季','S5赛季','S6赛季','S7赛季'}) do 
    ...
    function mt:on_add()
        ...
        for index,skill in ipairs(self.skill_book) do 
            if finds(skill.name,'S0赛季王者','S1赛季王者','S2赛季王者','S3赛季王者','S4赛季王者','S5赛季王者','S6赛季王者','S7赛季王者')then 
...
function mt:on_add()
    ...
    for index,skill in ipairs(self.skill_book) do 
        ...
        --     end
        -- end    
        if finds(skill.name,'S7赛季说明')then 
            skill:set_level(1)
        end   
    end 
end    
