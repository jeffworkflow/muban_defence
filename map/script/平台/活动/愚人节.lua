...
local function give_award(hero,unit) 
    ...
    local peon = p.peon
    local rand_list = award_list['四月傻瓜']
...
|cff00ffff在地上，埋下一颗傻瓜种子，数千年后可以收取蟠桃
...
function mt:on_cast_start()
    ...
    local p = hero:get_owner()
    local player = hero:get_owner()
    local u = p:create_unit('南瓜',self.target)
    u:set('生命',1)
    ...
    ac.wait((time+1)*1000,function()
        --创建蟠桃
        ac.item.create_item('南瓜',u:get_point())
        --移除桃树
        u:remove()
...
--获得事件
--[[傻瓜种子的获得途经：
1.神奇的5分钟，游戏开始5分钟，给每个玩家发放一个傻瓜种子，系统提示：华诞盛典普天同庆！XXXXX
2.每隔8分钟，基地区域随机生成一个傻瓜种子(一局12个)
3.打伏地魔获得35%概率掉落，每局限5个
4.挖宝0.75%概率触发，掉落5-10个随机傻瓜种子，每局限一次（参考碎片幼儿园），系统提示：华诞盛典普天同庆！XXXXX
]]
...
ac.game:event '游戏-开始'(function()
    ...
    ac.wait(time*1000,function()
        give_seed()
        ac.player.self:sendMsg('|cffffe799【系统消息】|r|cff00ff00盛世耀华夏，神州共欢腾！百花仙子大发慈悲，给所有玩家发放了一枚|cffff0000傻瓜种子|r',5) 
    end)
    ...
    ac.loop(time*1000,function()
        ...
        ac.item.create_item('傻瓜种子',point)
    end)
end)
...
ac.game:event '单位-死亡'(function(_,unit,killer)
    ...
    if p.kill_srm_guoshi < max_cnt and math.random(100) <= rate then 
        ac.item.create_item('傻瓜种子',unit:get_point())
